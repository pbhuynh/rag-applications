...
## Display properties
display_properties = ['guest', 'title', 'episode_url', 'length_seconds', 'thumbnail_url', 'summary', 'content']
...
def main(retriever: WeaviateWCS):
    ...
    with st.sidebar:
        ...
        retrieval_limit = st.slider('Retriever Limit', min_value=1, max_value=200, value=10, step=1)
        reranker_topk = st.slider('Reranker TopK', min_value=1, max_value=10, value=10, step=1)
    ...
    if query:
        ...
        hybrid_response = retriever.hybrid_search(query, collection_name, filter=guest_filter, alpha=alpha_input, return_properties=display_properties, limit=retrieval_limit)
